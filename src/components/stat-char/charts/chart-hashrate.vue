...
<template>
    <div class='grid-content bg-purple'>
      <div id='statChartsHashRate' class="statChartsClass" :style="{ 'min-width': '300px', width: '835px', height: '500px'}"></div>
    </div>
</template>
...
export default {
  mounted () {
    this.getHashRateChart(this.shardCharValue)
    this.drawStatChartsHashRate()
  },
  computed: {
    statData: {
      get () {
        return this.$store.state.chart.hashrateChart
      }
    },
    shardCharValue: {
      get () {
  ...
  methods: {
    ...mapActions(['getHashRateChart']),
    drawStatChartsHashRate () {
      var self = this
      // charts locate
      var statChartsElement = document.getElementById('statChartsHashRate')
      // charts instantiatte
      ...
      var option = {
        ...
        tooltip: {
          ...
          formatter: function (datas) {
            ...
            res += self.$t('statcharts.network.hashrateAverage') + ': ' + toopTipData.HashRate + '<br/>'
            return res
          },
          axisPointer: {
            type: 'cross'
        ...
        xAxis: {
          name: this.$t('statcharts.network.hashrateXname'),
          nameLocation: 'end',
          nameTextStyle: {
        ...
        series: [
          {
            name: this.$t('statcharts.network.hashrateName'),
            type: 'line',
            smooth: true,
            symbol: 'circle',
            symbolSize: 5,
