...
<template>
    <div class='grid-content bg-purple'>
      <div id='statChartsAccount' class="statChartsClass" :style="{ 'min-width': '300px', width: '835px', height: '500px'}"></div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(['getAddressChart']),
    drawStatChartsAccount () {
      var self = this
      // charts locate
      ...
      var statChartsDataX = function () {
        return self.addressChart.x
      }
      var statChartsDataY = function () {
        return self.addressChart.y
      }
      var statChartsTooltipData = function (index) {
        return self.addressChart.tooltip[index]
      }
      ...
      var option = {
        ...
        tooltip: {
          ...
          formatter: function (datas) {
            ...
            res += self.$t('statcharts.account.growthTotal') + ': ' + toopTipData.TotalAddresss + '<br/>'
            res += self.$t('statcharts.account.growthIncrease') + ': ' + toopTipData.DailyIncrease + '<br/>'
            return res
          },
          axisPointer: {
            type: 'cross'
        ...
        xAxis: {
          name: this.$t('statcharts.account.growthXname'),
          nameLocation: 'end',
          nameTextStyle: {
        ...
        series: [
          {
            name: this.$t('statcharts.account.growthName'),
            type: 'line',
            smooth: true,
            symbol: 'circle',
            symbolSize: 5,
