...
<template>
    <div class='grid-content bg-purple'>
      <div id='statChartsDifficulty' class="statChartsClass" :style="{ 'min-width': '300px', width: '835px', height: '500px'}"></div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(['getDifficultyChart']),
    drawStatChartsDifficulty () {
      var self = this
      // charts locate
      ...
      var option = {
        ...
        tooltip: {
          ...
          formatter: function (datas) {
            ...
            res += self.$t('statcharts.network.difficultyAverage') + ': ' + toopTipData.Difficulty + '<br/>'
            return res
          },
          axisPointer: {
            type: 'cross'
        ...
        xAxis: {
          name: this.$t('statcharts.network.difficultyXname'),
          nameLocation: 'end',
          nameTextStyle: {
        ...
        series: [
          {
            name: this.$t('statcharts.network.difficultyName'),
            type: 'line',
            smooth: true,
            symbol: 'circle',
            symbolSize: 5,
